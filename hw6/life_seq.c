...
{
    struct timeval start_overall, end_overall;
    count_start(&start_overall);
    ...
    if (argc == 5)
    {
        width = atoi(argv[3]);
        height = atoi(argv[4]);
        printf("Running %d iterations at %d by %d pixels using CPU.\n", iters, width, height);
    }
    else
    {
        width = WIDTH;
    ...
    printf("Total time spent: %f ms.\n", count_end(&start_overall, &end_overall));
